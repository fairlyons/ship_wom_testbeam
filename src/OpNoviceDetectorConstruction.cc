 ...
 : G4VUserDetectorConstruction()
{
  ...
  SctZ = SteelZ - WallThick*2 - Thickness_Reflect*2;
  ...
  Thickness_Gap = 1*mm;
  Thickness_Steel_Add_Top = 12*mm - Thickness_Hat;
  ...
  ybl = 601.561;
  ybr = 633.625;
  WOM_coord_vec = {{0*mm, 321*mm}, {0*mm, -321*mm}};
}
...
void OpNoviceDetectorConstruction::DefineMPTs()
{
  ...
  MPT_LAB_PPO->AddProperty("ABSLENGTH", photon_en_LAB_PPO_3, absLen_purified, 151, true, true);
...
void OpNoviceDetectorConstruction::DefineSurfaces()
{
  ...
  BaSO4_surface->SetModel(unified);
  ...
  for(int i = 0; i < 19; ++i) refl_test[i] = 1.0*refl_coating[i];
 
  MTP_BaSO->AddProperty("REFLECTIVITY", p_coating_refl, refl_coating, 19);

  BaSO4_surface->SetMaterialPropertiesTable(MTP_BaSO); 	
   
  G4LogicalSkinSurface* Surface1 = new G4LogicalSkinSurface("BaSO4_surface", ReflectBoxLV, BaSO4_surface);
...
void OpNoviceDetectorConstruction::DefineSolids()
{
  ...
  // Reflectivity box
  std::vector<G4TwoVector> reflect =
  {G4TwoVector(-x+WallThick, -ybl+WallThick),
   ...
   G4TwoVector(x-WallThick, ytr-WallThick),
   G4TwoVector(x-WallThick, -ybr+WallThick)};
  G4GenericTrap* ReflectBox = new G4GenericTrap("Reflect_Box", ReflectZ/2., reflect);

  // Scintillator box
  std::vector<G4TwoVector> scint = 
  {G4TwoVector(-x+WallThick+Thickness_Reflect, -ybl+WallThick+Thickness_Reflect),
   G4TwoVector(-x+WallThick+Thickness_Reflect, ytl-WallThick-Thickness_Reflect),
   G4TwoVector(x-WallThick-Thickness_Reflect, ytr-WallThick-Thickness_Reflect),
   G4TwoVector(x-WallThick-Thickness_Reflect, -ybr+WallThick+Thickness_Reflect),
   G4TwoVector(-x+WallThick+Thickness_Reflect, -ybl+WallThick+Thickness_Reflect),
   G4TwoVector(-x+WallThick+Thickness_Reflect, ytl-WallThick-Thickness_Reflect),
   G4TwoVector(x-WallThick-Thickness_Reflect, ytr-WallThick-Thickness_Reflect),
   G4TwoVector(x-WallThick-Thickness_Reflect, -ybr+WallThick+Thickness_Reflect)};
  ...
  SteelAdd = new G4Tubs("Steel_Add", Rin, Rout, Thickness_Steel_Add_Bot/2, 0, 360*deg);
  // LAB&PPO inside tube
  Rin = 0.0*mm;
  Rout = Diam_In_In/2;
  SctInside = new G4Tubs("Sct_Inside", Rin, Rout, (Length_In + Thickness_Ring)/2, 0, 360*deg);

  G4SubtractionSolid* EmptySteelBox = new G4SubtractionSolid("Empty_Steel_Box", SteelBox, ReflectBox, 0, G4ThreeVector(0, 0, 0));
  G4SubtractionSolid* EmptyReflectBox = new G4SubtractionSolid("Empty_Reflect_Box", ReflectBox, ScintillatorBox, 0, G4ThreeVector(0, 0, 0));
  ...
  G4double delta_Z_Steel_Add = SteelZ/2 + Thickness_Steel_Add_Bot/2;
  ...
  for(unsigned int pos = 0; pos<WOM_coord_vec.size(); pos++) {
    ...
    if(pos == 0) {
      ...
      EmptyReflectBoxWithHole_tempvec.push_back(new G4SubtractionSolid("Empty_Reflect_Box_With_Hole_0", EmptyReflectBox, HoleBox, t_SteelHole));
      ...
      EmptyReflectBoxWithHole_tempvec.push_back(new G4SubtractionSolid("Empty_Reflect_Box_With_Hole_1", EmptyReflectBoxWithHole_tempvec.back(), HoleBox, t_SteelHole));
  ...
  EmptyReflectBoxWithHole = EmptyReflectBoxWithHole_tempvec.back();
...
void OpNoviceDetectorConstruction::DefineLogicalVolumes()
{
  ...
  ReflectBoxLV = new G4LogicalVolume(EmptyReflectBoxWithHole, BaSO4, "Reflect_Box_LV", 0, 0, 0);
...
void OpNoviceDetectorConstruction::ConstructVolumes()
{
  ...
  ReflectBoxPV = new G4PVPlacement(0, G4ThreeVector(), ReflectBoxLV, "Reflect_Box_PV", ExpHallLV, false, 1000, intersect_check);
...
void OpNoviceDetectorConstruction::DefineVisAttributes()
{
  ...
  ReflectBoxLV->SetVisAttributes(steelBoxVisAtt);
