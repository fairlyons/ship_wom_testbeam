...
void OpNoviceDetectorConstruction::DefineMPTs()
{
 ...
 /* G4double pmma_side_abslen_vlad[pmma_mpt_entr] = {10.55*mm,18.23*mm,24.6*mm,36.07*mm,39.7*mm,42.6*mm,43.69*mm,45.77*mm,52.97*mm,61.48*mm,66.44*mm,70.39*mm,79.11*mm};
  ...
  // BIS reemission
  G4double waveLength4[52] =   {498.50394264782,495.512169509067,489.3462170527,484.343493743363,479.225925762846,472.348366539066,467.849074868481,464.04990973537,461.338683704906,459.793161903923,457.56412302422,454.795680527534,
  ...
  G4double photonEnergy6[52];
  for(int i=0; i < sizeof(photonEnergy6)/sizeof(photonEnergy6[0]); i++) photonEnergy6[i] = 1240./waveLength4[i]*eV;
  G4double reEmit4[52] = {0.095367125872878,0.100714589065125,0.13933792816855,0.176288041362039,0.211928140548633,0.245505005810625,0.273370328546945,0.30221148342213,0.333093045311687,0.364223283871252,0.399221677160171,
  ...
  MPT_WLSCoat->AddProperty("WLSCOMPONENT", photonEnergy6, reEmit4, 52);
...
void OpNoviceDetectorConstruction::DefineLogicalVolumes()
{
  ...
  ReflectBox_log = new G4LogicalVolume(EmptyReflectBoxWithHole, LAB_PPO,"Reflectbox", 0,0,0);
