...
void OpNoviceDetectorConstruction::DefineMPTs()
{
  ...
  for(int i = 0; i < int(sizeof(photonWaveLength3)/sizeof(photonWaveLength3[0])); i++) photon_en_LAB_PPO_2[i] = 1240./photonWaveLength3[i]*eV;
  ...
  for(int i = 0; i < int(sizeof(waveLength)/sizeof(waveLength[0])); i++) photon_en_LAB_PPO_3[i] = 1240./waveLength[i]*eV;
  ...
  for(int i = 0; i < int(sizeof(pmma_bottom_en)/sizeof(pmma_bottom_en[0])); i++ ) pmma_bottom_en[i]=1240./pmma_bottom_wl[i]*eV;
  ...
  for(int i = 0; i < int(sizeof(photonEnergy5)/sizeof(photonEnergy5[0])); i++) photonEnergy5[i] = 1240./waveLength3[i]*eV;
  ...
  for(int i = 0; i < int(sizeof(photonEnergy6)/sizeof(photonEnergy6[0])); i++) photonEnergy6[i] = 1240./waveLength4[i]*eV;
...
void OpNoviceDetectorConstruction::DefineSurfaces()
{
  ...
  G4double reflectivity2[2] = {0.0999, 0.0999}; // by A. K.

  G4MaterialPropertiesTable *MPTsurf_SipmWindowSurface = new G4MaterialPropertiesTable();
  MPTsurf_SipmWindowSurface->AddProperty("REFLECTIVITY", pp1, reflectivity2, 2);
  ...
  for(int i = 0; i < int(sizeof(photonEnergy7)/sizeof(photonEnergy7[0])); i++) photonEnergy7[i] = 1240./waveLength5[i]*eV;
 ...
 11.7364909186509,12.943617756497,38.1908081955548,16.6488888058538,19.1187061362638,26.7334655790205,29.7746096076586,        35.7572000618953,36.8950525383479,38.834406796401,40.2088285102441,36.333977297541,31.4552729744343,31.7565672164803,
  ...
  for(int i = 0; i < int(sizeof(photonEnergy8)/sizeof(photonEnergy8[0])); i++) photonEnergy8[i] = 1240./waveLength6[i]*eV;
...
//-------------------------------------------------------------------
  ...
  for(unsigned int pos = 0; pos < WOM_coord_vec.size(); pos++) {
    ...
    WLS_tube1_phys_vect.push_back( new G4PVPlacement(RM1, G4ThreeVector(0.,0., 0.), WLS_tube1_log, "WLS1", WOM_cell_log, false, 105, intersect_check) );
    WLS_tube2_phys_vect.push_back( new G4PVPlacement(RM1, G4ThreeVector(0.,0., 0.), WLS_tube2_log, "WLS2", WOM_cell_log, false, 107, intersect_check) );
