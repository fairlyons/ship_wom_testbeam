 ...
 : G4VUserDetectorConstruction()
{
  ...
  Diam_WOM_Out = 60*mm;
  Thickness_WOM = 3*mm;
  Diam_WOM_In = Diam_WOM_Out - 2*Thickness_WOM;
  Thickness_Air = 2*mm;
  Diam_Out_In = Diam_WOM_Out + 2*Thickness_Air;
  Thickness_Vessel = 3*mm;
  Diam_Out_Out = Diam_Out_In + 2*Thickness_Vessel;
  Diam_In_Out = Diam_WOM_In - 2*Thickness_Air;
  Diam_In_In = Diam_In_Out - 2*Thickness_Vessel;
...
void OpNoviceDetectorConstruction::DefineMPTs()
{
  ...
  //------------------------------------------------------------------------------
  // Rindex LABPPO
  G4double rindex_LABPPO[100];
  G4double photon_RindLABPPO[100];
  auto Rindex_LABPPO = [=](G4double wl) {
    G4double rind = 1.;
    G4double B[4], C[4];
  ...
  MPT_LAB_PPO->AddProperty("RINDEX", photon_RindLABPPO, rindex_LABPPO, 100);//->SetSpline(true);
  MPT_LAB_PPO->AddProperty("SCINTILLATIONCOMPONENT1", photon_EmisLABPPO, emission_LABPPO, 201, false, true);
  MPT_LAB_PPO->AddProperty("SCINTILLATIONCOMPONENT2", photon_EmisLABPPO, emission_LABPPO, 201, false, true);
  //MPT_LAB_PPO->AddProperty("ABSLENGTH", photon_AbsLABPPO22, abslen_LABPPO22Purified, 151, true, true);
  MPT_LAB_PPO->AddProperty("ABSLENGTH", photon_AbsLABPPOApr24, abslen_LABPPOApr24, 300, true, false); // Spline causes warnings
  ...
  MPT_PMMA_bottom->AddProperty("RINDEX", photon_RindPMMA, rindex_PMMA, 13, true, true);
  MPT_PMMA_bottom->AddProperty("ABSLENGTH", photon_AbsPMMABot, abslen_PMMABot, 37, true, false);
  ...
  MPT_WLSCoat->AddProperty("WLSABSLENGTH", photon_AbsWLS, abslen_WLS, 272);  // jakob thesis -- without BPEA 
  MPT_WLSCoat->AddProperty("WLSCOMPONENT", photon_EmisWLS, emission_WLS, 51);
  MPT_WLSCoat->AddConstProperty("WLSTIMECONSTANT", 2.*ns); // More or less it should be this value
  MPT_WLSCoat->AddProperty("RINDEX", photon_RindPMMA, rindex_PMMA, 13, true, true); //??
  ...
  //------------------------------------------------------------------------------
  // Rindex Air
  G4double photon_RindAir[2] = {2.*eV, 5.*eV};
  G4double rindex_Air[2] = {1.00, 1.00};

  G4MaterialPropertiesTable* MPT_Air = new G4MaterialPropertiesTable();
  MPT_Air->AddProperty("RINDEX", photon_RindAir, rindex_Air, 2);
  ...
  //----------------------------- Silicon Resin  -----------------------------
  //------------------------------------------------------------------------------
  // Rindex Silicon Resin
  G4double photon_RindResin[3] = {2.*eV, 4.*eV, 5.*eV};
  G4double rindex_Resin[3] = {1.57, 1.57, 1.57}; //Hamamatsu data sheet

  G4MaterialPropertiesTable* MPT_Resin = new G4MaterialPropertiesTable();
  MPT_Resin->AddProperty("RINDEX", photon_RindResin, rindex_Resin, 3);
  ResinSi->SetMaterialPropertiesTable(MPT_Resin);
  ...
  //------------------------------------------------------------------------------
  // Rindex Silicon
  G4double photon_RindSi[3] = {2.76*eV,2.88*eV,3.10*eV};
  G4double rindex_Si[3] = {2.59,2.75,2.91}; //from article 2002.04218 

  G4MaterialPropertiesTable* MPT_Si = new G4MaterialPropertiesTable(); 
  MPT_Si->AddProperty("RINDEX", photon_RindSi, rindex_Si, 3);
...
void OpNoviceDetectorConstruction::DefineSurfaces()
{
  ...
  // Reflectivity Cell
  G4double photon_ReflSteel[2] = {1*eV, 5*eV};
  G4double other_coating[2] = {0, 0}; // it is not relevant in our case, it is for not smooth surfaces 
  G4double relfection_TotalSteel[2] = {0.25, 0.25};

  G4MaterialPropertiesTable *MPTsurf_Steel = new G4MaterialPropertiesTable();
  MPTsurf_Steel->AddProperty("REFLECTIVITY", photon_ReflSteel, relfection_TotalSteel, 2);
  ...
  BaSO4_surfaceLT->SetModel(unified);
  ...
  BaSO4_surfaceRT->SetModel(unified);
  ...
  BaSO4_surfaceLB->SetModel(unified);
  ...
  BaSO4_surfaceRB->SetModel(unified);
  
  // Specular Reflectivity of Coating
  G4double wavelength_SpecCoat[59] = {837.9882615,828.5013155,818.487317,807.9462659,798.45932,787.9182689,778.4313229,767.8902719,758.9303785,748.3893274,739.429434,727.8342778,717.8202793,709.3874384,697.2652297,687.2512312,678.8183903,668.8043918,658.7903933,648.7763948,638.2353437,627.6942927,618.7343992,609.2474533,598.7064022,589.2194563,580.2595628,568.1373541,558.1233556,546.0011469,537.568306,528.6084126,518.0673615,508.053363,499.6205222,487.4983134,478.0113675,467.997369,458.510423,447.9693719,439.5365311,427.4143223,418.4544289,408.4404304,399.480537,387.8853808,378.3984349,368.3844363,357.8433853,347.8293868,338.3424408,327.8013897,318.3144438,308.8274978,299.3405518,287.7453957,278.7855023,269.8256089,258.2304527}; 
  G4double photon_SpecCoat[59];
  for(unsigned int i = 0; i < sizeof(photon_SpecCoat)/sizeof(photon_SpecCoat[0]); i++) photon_SpecCoat[i] = 1240./wavelength_SpecCoat[i]*eV;
  G4double specular_Coat[59] = {0.01827689,0.006247856,0.016558456,0.00796629,0.002810989,0.011403156,0.00796629,0.01312159,0.004529423,0.01312159,0,0.014840023,0.00796629,0,0,0.011985604,0.003428249,0.008562662,0.008562662,0.008562662,0.003428249,5.30684E-06,0,0.010274133,0.010942009,0,0.010942009,0.009231693,0.000680113,0.010942009,0,0.004100745,0,0.005811061,0.000680113,0.002699077,0.007819659,0.01123338,0.000992216,0.000992216,0,0.007819659,0,0.004405937,0.006781677,0.003370253,0.006781677,0.008487389,0.00857075,0.010279258,0,0,0,0,0,0.001514251,0.001514251,0,0}; // specular reflection Patrick data, calculated as 1- diffuse reflection since the specular was very small and the measurements for specular are more difficult to separete from the diffuse ones.

  // Total Reflectivity of Coating
  G4double photon_ReflCoat[19] = {0.653775342148272*eV,0.687758866578874*eV,0.729108650377501*eV,0.773110023703728*eV,0.827676607343705*eV,0.887209379832684*eV,0.954680932864894*eV,1.03471853868547*eV,1.12204531460793*eV,1.24533694371132*eV,1.3767339264975*eV,1.55649354301667*eV,1.77189963551006*eV,2.09375317772178*eV,2.51208306681883*eV,3.16797881984726*eV,3.50425046717685*eV,3.63215740198218*eV,3.81157924082758*eV};
  G4double reflection_Coat[19] = {0.861320132013201,0.916105610561056,0.922706270627063,0.920726072607261,0.916765676567657,0.931287128712871,0.95042904290429,0.958349834983498,0.962970297029703,0.968250825082508,0.969570957095709,0.973531353135314,0.977491749174918,0.978151815181518,0.98013201320132,0.980792079207921,0.970891089108911,0.960990099009901,0.953729372937294}; // reflectivity of the coating https://www.optopolymer.de/produktuebersicht/diffuse-reflecting-materials/bariumsulfate-baso4-coating-oprc/

  G4double reflLT[19], reflRT[19], reflLB[19], reflRB[19];
  for(int i = 0; i < 19; ++i) {
    reflLT[i] = 1.0*reflection_Coat[i];
    reflRT[i] = 1.0*reflection_Coat[i];
    reflLB[i] = 1.0*reflection_Coat[i];
    reflRB[i] = 1.0*reflection_Coat[i];
  }
 
  G4MaterialPropertiesTable* MTP_BaSOLT = new G4MaterialPropertiesTable();
  MTP_BaSOLT->AddProperty("SPECULARLOBECONSTANT", photon_SpecCoat, specular_Coat, 59); // In order to have diffuse reclectivity (Lambertian), it is necessary define all the other three.
  MTP_BaSOLT->AddProperty("SPECULARSPIKECONSTANT", photon_ReflSteel, other_coating, 2); //  The diffuse is 1-other three (in this case 1-specular).
  MTP_BaSOLT->AddProperty("BACKSCATTERCONSTANT", photon_ReflSteel, other_coating, 2);
  MTP_BaSOLT->AddProperty("REFLECTIVITY", photon_ReflCoat, reflLT, 19);
  G4MaterialPropertiesTable* MTP_BaSORT = new G4MaterialPropertiesTable();
  MTP_BaSORT->AddProperty("SPECULARLOBECONSTANT", photon_SpecCoat, specular_Coat, 59); // In order to have diffuse reclectivity (Lambertian), it is necessary define all the other three.
  MTP_BaSORT->AddProperty("SPECULARSPIKECONSTANT", photon_ReflSteel, other_coating, 2); //  The diffuse is 1-other three (in this case 1-specular).
  MTP_BaSORT->AddProperty("BACKSCATTERCONSTANT", photon_ReflSteel, other_coating, 2);
  MTP_BaSORT->AddProperty("REFLECTIVITY", photon_ReflCoat, reflRT, 19);
  G4MaterialPropertiesTable* MTP_BaSOLB = new G4MaterialPropertiesTable();
  MTP_BaSOLB->AddProperty("SPECULARLOBECONSTANT", photon_SpecCoat, specular_Coat, 59); // In order to have diffuse reclectivity (Lambertian), it is necessary define all the other three.
  MTP_BaSOLB->AddProperty("SPECULARSPIKECONSTANT", photon_ReflSteel, other_coating, 2); //  The diffuse is 1-other three (in this case 1-specular).
  MTP_BaSOLB->AddProperty("BACKSCATTERCONSTANT", photon_ReflSteel, other_coating, 2);
  MTP_BaSOLB->AddProperty("REFLECTIVITY", photon_ReflCoat, reflLB, 19);
  G4MaterialPropertiesTable* MTP_BaSORB = new G4MaterialPropertiesTable();
  MTP_BaSORB->AddProperty("SPECULARLOBECONSTANT", photon_SpecCoat, specular_Coat, 59); // In order to have diffuse reclectivity (Lambertian), it is necessary define all the other three.
  MTP_BaSORB->AddProperty("SPECULARSPIKECONSTANT", photon_ReflSteel, other_coating, 2); //  The diffuse is 1-other three (in this case 1-specular).
  MTP_BaSORB->AddProperty("BACKSCATTERCONSTANT", photon_ReflSteel, other_coating, 2);
  MTP_BaSORB->AddProperty("REFLECTIVITY", photon_ReflCoat, reflRB, 19);
  ...
  // Reflectivity SiPM
  G4double wavelength_ReflSiPM[2] = {1000, 100};
  G4double photon_ReflSiPM[2];
  for(unsigned int i = 0; i < sizeof(photon_ReflSiPM)/sizeof(photon_ReflSiPM[0]); i++) photon_ReflSiPM[i] = 1240./wavelength_ReflSiPM[i]*eV;
  G4double reflection_sipm[2] = {0, 0}; // we want that the surface of the sipms sides is completely absorbent
       
  G4MaterialPropertiesTable* MTP_sipms = new G4MaterialPropertiesTable();
  MTP_sipms->AddProperty("REFLECTIVITY", photon_ReflSiPM, reflection_sipm, 2);
...
void OpNoviceDetectorConstruction::DefineSolids()
{
  ...
  G4SubtractionSolid* SteelBeamVWithHoles = SteelBeamV_tempvec.back();
  ...
  G4SubtractionSolid* SteelBeamHWithHoles = SteelBeamH_tempvec.back();
  // Unified beams
  ...
  std::vector<G4SubtractionSolid*> EmptySteelBox_tempvec;
  EmptySteelBox_tempvec.push_back(new G4SubtractionSolid("EmptySteelBox", SteelBox, ReflectBox, 0, G4ThreeVector(0, 0, 0)));
  EmptySteelBox_tempvec.push_back(new G4SubtractionSolid("EmptySteelBox", EmptySteelBox_tempvec.back(), ReflectBoxLT, 0, G4ThreeVector(0, 0, 0)));
  EmptySteelBox_tempvec.push_back(new G4SubtractionSolid("EmptySteelBox", EmptySteelBox_tempvec.back(), ReflectBoxRT, 0, G4ThreeVector(0, 0, 0)));
  EmptySteelBox_tempvec.push_back(new G4SubtractionSolid("EmptySteelBox", EmptySteelBox_tempvec.back(), ReflectBoxLB, 0, G4ThreeVector(0, 0, 0)));
  EmptySteelBox_tempvec.push_back(new G4SubtractionSolid("EmptySteelBox", EmptySteelBox_tempvec.back(), ReflectBoxRB, 0, G4ThreeVector(0, 0, 0)));
  G4SubtractionSolid* EmptySteelBox = EmptySteelBox_tempvec.back();
